...
#[test]
fn space_talking_cat_move_hh10_video_to_8s_keeps_unsynced_audio_in_place() {
    let otio = std::fs::read_to_string(
        std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
            .join("tests/fixtures/space_talking_cat_move_hh10.otio"),
    )
    .expect("fixture exists");
    let mut timeline: Timeline = serde_json::from_str(&otio).expect("parse otio");
    timeline.tracks.sanitize();

    let audio_start_before = {
        let (ti, ii, _) = timeline.tracks.get_item("5a211e26ecb1").unwrap();
        timeline.tracks.children[ti].start_time_of_item(ii)
    };
    ...
    let (vti, vii, _) = timeline.tracks.get_item("9d4146e883f4").unwrap();
    let video_start = timeline.tracks.children[vti].start_time_of_item(vii);
    assert!((video_start - 8.0).abs() < 1e-6);

    let (ati, aii, _) = timeline.tracks.get_item("5a211e26ecb1").unwrap();
    let audio_start = timeline.tracks.children[ati].start_time_of_item(aii);
    assert!(
        (audio_start - audio_start_before).abs() < 1e-6,
        "unsynced HH10 audio must not move with video: {audio_start} vs {audio_start_before}"
    );
}

#[test]
fn space_talking_cat_move_linked_hh10_video_to_8s_moves_audio_partner() {
    const HH10_GROUP: i64 = 5;
    ...
    for id in ["9d4146e883f4", "5a211e26ecb1"] {
        let (ti, ii, _) = timeline.tracks.get_item(id).unwrap();
        if let Item::Clip(clip) = &mut timeline.tracks.children[ti].items[ii] {
            clip.metadata["Resolve_OTIO"] = serde_json::json!({ "Link Group ID": HH10_GROUP });
        }
    }
    ...
    let (ati, aii, _) = timeline.tracks.get_item("5a211e26ecb1").unwrap();
    let audio_start = timeline.tracks.children[ati].start_time_of_item(aii);
    assert!((video_start - 8.0).abs() < 1e-6, "video at {video_start}");
    assert!(
        (audio_start - 8.0).abs() < 1e-6,
        "linked HH10 audio must land with video at 8s, got {audio_start}"
    );
    assert!(
        timeline.tracks.get_item("5a211e26ecb1").is_some(),
        "linked HH10 audio must keep its timeline id"
    );
}